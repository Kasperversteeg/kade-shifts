...
const submit = (): void => {
    ...
    form.post(route('admin.entries.reject', props.entryId), {
        onSuccess: () => {
            emit('close');
        },
    });
...
<template>
    <dialog ref="dialogRef" class="modal" @close="emit('close')">
        <div class="modal-box">
            <h3 class="text-lg font-bold">{{ t('admin.reject') }}</h3>
            <form @submit.prevent="submit" class="mt-4">
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">{{ t('admin.rejectionReason') }}</legend>
                    <textarea
                        v-model="form.reason"
